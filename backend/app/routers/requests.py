...
def save_route_legs(request_id: str, legs: list[RouteLegCreate], db: Session = Depends(get_db)):
    ...
    for i, leg in enumerate(legs):
        rl = RouteLeg(
            ...
            distance_km=leg.distance_km, duration_min=leg.duration_min, return_trip=leg.return_trip,
        )
        db.add(rl)
        new_legs.append(rl)
    req.updated_at = datetime.now(timezone.utc)
...
def save_allowance_days(request_id: str, days: list[DailyAllowanceDayCreate], db: Session = Depends(get_db)):
    ...
    for d in days:
        ad = DailyAllowanceDay(
            id=str(uuid.uuid4()), request_id=request_id,
            day=d.day, is_abroad=d.is_abroad,
            meal_breakfast=d.meal_breakfast, meal_lunch=d.meal_lunch, meal_dinner=d.meal_dinner,
