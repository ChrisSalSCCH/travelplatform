...
class ProjectCreate(BaseModel):
    ...
    def funder_valid(cls, v: str) -> str:
        if v not in ("FFG", "FWF", "CDG", "OTHER"): raise ValueError("funder must be FFG, FWF, CDG or OTHER")
        return v
...
class RouteLegResponse(BaseModel):
    ...
    return_trip: bool; created_at: datetime
...
class ExpenseItemCreate(BaseModel):
    ...
    km: Optional[Decimal] = None; amount: Decimal
    receipt_url: Optional[str] = None
    @field_validator("category")
    @classmethod
    def category_valid(cls, v: str) -> str:
        allowed = ("daily_allowance", "mileage", "accommodation", "transport", "other")
        if v not in allowed: raise ValueError(f"category must be one of {allowed}")
        return v
    @field_validator("description")
    @classmethod
    def desc_max(cls, v: str) -> str:
        if len(v) > 400: raise ValueError("description max 400 characters")
        return v
...
class TravelRequestCreate(BaseModel):
    ...
    def max_500(cls, v: str) -> str:
        if len(v) > 500: raise ValueError("max 500 characters")
        return v
...
class TravelRequestUpdate(BaseModel):
    ...
    work_package: Optional[str] = None
    trip_start: Optional[date] = None; trip_end: Optional[date] = None
    departure_time: Optional[datetime] = None; return_time: Optional[datetime] = None
    meal_breakfast: Optional[bool] = None; meal_lunch: Optional[bool] = None
    meal_dinner: Optional[bool] = None; project_id: Optional[str] = None
